...
export function getPolicyLabels(world: World, factionId: number) {
  ...
  return {
    ...
    recruitment: `Towns employ up to <b style="color:#e2e8f0">${policy.recruitment}x</b> workers (capped by jobs). In war, recruit <b style="color:#e2e8f0">${policy.recruitment}x</b> the soldier target.`,
    ...
    rations: `Each citizen eats <b style="color:#e2e8f0">${policy.rations}x</b> food.${policy.rations < 0.8 ? ' <span style="color:#e74c3c">⚠ Below 0.8x growth drops 10%.</span>' : ''}`,
